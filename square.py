...
def attack(model, x, y, corr, y_pred, y_undefended, l2, eps, n_iters, stop_iters, p_init, num_s, batch_size, targeted, loss_type, resume_path, plot, period):
    ...
    while n_queries.max() < stop_iters and acc != 0:
        ...
        idx_improved_exp = (idx_improved_up) if torch.rand(1).item() < 0.85 else (idx_improved_down)
        idx_improved_trans = (np.floor(margin) < np.floor(margin_min_curr)) + (np.floor(margin + 0.5) < np.floor(margin_min_curr + 0.5)) + (np.floor(margin -0.5) < np.floor(margin_min_curr - 0.5))# + (np.floor(margin + 0.1) < np.floor(margin_min_curr + 0.1))
